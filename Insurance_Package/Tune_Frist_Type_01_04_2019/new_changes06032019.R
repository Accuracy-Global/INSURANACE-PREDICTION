...
ui <- shinyUI(fluidPage(
  # Application title
  h1("Tune Project Demo App", align = "center"),
  fluidRow(
    
    column(width=6,
           fluidRow( style = "height:300px; width: 400px;border-style: groove; background-color: #85C1E9; padding-left: 50px; padding-top: 25px; margin-left: 50px;", actionButton("goButton", "STEP1",color = "primary"),
                     ...
                     radioButtons("gender","select gender",list("M","F"),""), # Input as M or F
                     textInput("tripdays","Trip duration","")),
           fluidRow(style = "height:300px; width: 400px;padding-left: 50px; margin-left: 50px;margin-top: 15px; border-style: groove;background-color: #85C1E9;padding-top: 25px;",actionButton("goButton", "STEP2"),
                    selectInput("channel","select channel",c("WEB","APIMOBILEAPP"),""), # Type of booking
                    selectInput("nationality","select nationality",c("Australia","Indonesian","German","Russian","Iran","Indian","Italian","Turkish","American","Japan","Greek","Kuwait","New Zealand","Pakistan","Bangladesh","Canada","Italy","Nepal"),""),
                    selectInput("connectingtype","connecting type",c("Direct Flight","Connecting Flight","Normal"),"")), # Booking flight type
           
           fluidRow(style = "height:300px; padding-left: 220px; ",submitButton("submit"))),
    column(width = 6,
           fluidRow( style = "height:auto; background-color: white; margin-left: -20px;",h4("Insured Probabilities:"),
...
server <- function(input, output) {
  ...
  output$prediction <- renderPlot({ 
    ...
    df <- readxl::read_excel('experiment.xlsx')
    ...
    m=polr(Insured ~ Psg_Age + TripDurationDays + ConnectingFlightType + Channel + Psg_Gender + Nationality, data = df, Hess = TRUE)
    #Here mapping the user input with trained model
    n1 = data.frame(Psg_Age = age1, TripDurationDays = trip1, ConnectingFlightType = connecting1, Channel = channel1,Psg_Gender = gender1, Nationality = nationality1 )
